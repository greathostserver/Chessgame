...
function fenFromBoard(){
  // Build FEN from current board only (no clocks/castling for engine; we keep simple)
...
function attemptMove(fromIdx,toIdx){
  ...
  // Track LAN for engine: from-to + promotion if any
...
function onBestMoveHandler(bestmove){
  // bestmove is LAN like "e2e4" or "e7e8q"
  // Convert LAN to our indices
  ...
  const mv = legalMoves(state.whiteToMove).find(m=>m.from===fromIdx&&m.to===toIdx) || null;

  if(!mv){
    // Handle promotions from engine (e.g., e7e8q)
    if(bestmove.length>=5){
      ...
      if(candidate) {
        finalizeAIMove(candidate, bestmove);
      }
    }
    return;
  }
  finalizeAIMove(mv, bestmove);
}
function finalizeAIMove(mv, lan){
  const san=moveToSAN(mv);
  makeMove(mv);
  state.historySAN.push(san);
  uiState.moveListLAN.push(lan);
...
function boot(){
  // Engine init
  initEngine();
  setOptions({Threads: 1, Hash: 32}); // lightweight defaults; تو می‌توانی بیشتر کنی
  ...
  // Events
  dom.newBtn.addEventListener('pointerup',()=>{loadFEN(initialFEN); uiState.moveListLAN.length=0; renderBoard(); renderHistory(); setUndoEnabled(false); showModal(i18n[uiState.lang].applied,'ok');});
  ...
  dom.undoBtn.addEventListener('pointerup',()=>{
    ...
    const ok = (function(){ // custom undo that also trims LAN
      const lastLAN = uiState.moveListLAN.pop();
      return lastLAN!==undefined;
    })();
    if(ok){
      // rules.undo handles board and SAN
      // Note: for simplicity we reload initial and reapply LAN after full undo levels if needed. Here we just rely on rules.undo.
    }
  ...
  dom.applyLang.addEventListener('pointerup',()=>{
    uiState.lang=dom.langSel.value; localStorage.setItem('chess_lang',uiState.lang); applyLang(); renderBoard(); renderHistory(); showModal(i18n[uiState.lang].applied,'ok');
  });
